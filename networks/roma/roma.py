...
class RegressionMatcher(nn.Module):
    ...
    def match(
        ...
        with torch.no_grad():
            ...
            ws = self.w_resized
            hs = self.h_resized
            im_A = F.interpolate(im_A_path, size=(hs, ws), mode='bilinear', align_corners=False)
            im_B = F.interpolate(im_B_path, size=(hs, ws), mode='bilinear', align_corners=False)
            ...
            if self.upsample_preds:
                ...
                im_A = F.interpolate(im_A_path, size=(hs, ws), mode='bilinear',
                                     align_corners=False)
                im_B = F.interpolate(im_B_path, size=(hs, ws), mode='bilinear',
                                     align_corners=False)
            ...
            # remove black pixels
            black_mask1 = (im_A_path[0, 0] < 0.03125) & (im_A_path[0, 1] < 0.03125) & (im_A_path[0, 2] < 0.03125)
            black_mask2 = (im_B_path[0, 0] < 0.03125) & (im_B_path[0, 1] < 0.03125) & (im_B_path[0, 2] < 0.03125)
            black_mask1 = F.interpolate(black_mask1.float()[None, None], size=tuple(certainty.shape[-2:]), mode='nearest').bool()
            black_mask2 = F.interpolate(black_mask2.float()[None, None], size=tuple(certainty.shape[-2:]), mode='nearest').bool()
...
def RoMa(img_size, pretrained_backbone=False, **kwargs):
    gp_dim = 512
    feat_dim = 512
